...
def smooth_poses_gaussian(poses, sigma=2.0):
    smoothed = poses.copy()
...
def fit_sequence_single_pass(
    ...
    for it in range(iters):
        ...
        # Get joints WITH offsets applied (joint + offset ≈ marker)
        pred_markers = fitter()

        # Select only the joints that have markers
        pred_markers_selected = pred_markers[:, targets_idx, :]

        per_elem = huber(pred_markers_selected, markers_torch)
        marker_loss = per_elem.mean()

        pose_l2 = (fitter.body_pose**2).mean()
        betas_l2 = (fitter.betas**2).mean()
        offsets_l2 = (fitter.marker_offsets**2).mean()
        ...
        # Get joints WITHOUT offsets for foot orientation calculation
        joints_no_offsets = fitter(return_joints_only=True)

        l_ankle_pos = joints_no_offsets[:, J["L_ankle"], :]
        r_ankle_pos = joints_no_offsets[:, J["R_ankle"], :]
        l_foot_pos = joints_no_offsets[:, J["L_foot"], :]
        r_foot_pos = joints_no_offsets[:, J["R_foot"], :]
        ...
        l_hip_pos = joints_no_offsets[:, J["L_hip"], :]
        r_hip_pos = joints_no_offsets[:, J["R_hip"], :]
        pelvis_pos = joints_no_offsets[:, J["pelvis"], :]
        spine1_pos = joints_no_offsets[:, J["spine1"], :]
        ...
        if verbose and ((it + 1) % 100 == 0 or it == 0):
            print(
                f"    iter {it + 1:03d}: loss={loss.item():.5f}  marker={marker_loss.item():.5f}  offsets={offsets_l2.item():.5f}"
            )
    ...
    for subj in subjects:
        ...
        if init_betas is None:
            ...
            result = fit_sequence(
                ...
                w_offsets=1e-2,  # Default offset regularization
        ...
        for npz_path in npz_files:
            ...
            np.savez(
                ...
                marker_offsets=result.get("marker_offsets", np.zeros((24, 3))),
            )

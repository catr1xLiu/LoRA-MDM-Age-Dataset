...
class SMPLFitterV3(nn.Module):
    ...
    def __init__(self, smpl_layer, T, device="cpu", init_betas=None):
        ...
        betas_init = torch.zeros(10, device=device) if init_betas is None \
            else torch.as_tensor(init_betas, device=device).float()
...
def fit_sequence_single_pass(fitter, markers_torch, targets_idx, travel_direction_torch, travel_speed_torch,
                              device, iters, w_marker, w_pose, w_betas,
                              w_transl_vel, w_transl_acc, w_pose_vel, w_pose_acc,
                              w_spine_vel, w_spine_acc, w_foot_orient, w_foot_smooth,
                              verbose=True):
    ...
    for it in range(iters):
        ...
        transl_acc = torch.tensor(0., device=device)
        if T > 2:
        ...
        pose_acc = torch.tensor(0., device=device)
        orient_acc = torch.tensor(0., device=device)
        if T > 2:
            pose_acc = (fitter.body_pose[2:] - 2*fitter.body_pose[1:-1] + fitter.body_pose[:-2]).pow(2).mean()
            orient_acc = (fitter.global_orient[2:] - 2*fitter.global_orient[1:-1] + fitter.global_orient[:-2]).pow(2).mean()
        
        spine_pose = fitter.body_pose[:, spine_pose_indices]
        spine_vel = (spine_pose[1:] - spine_pose[:-1]).pow(2).mean()
        spine_acc = torch.tensor(0., device=device)
        if T > 2:
        ...
        foot_acc = torch.tensor(0., device=device)
        if T > 2:
        ...
        foot_orient_loss = torch.tensor(0., device=device)
...
def fit_sequence(smpl, markers_torch, marker_joint_idx, used_names, init_betas, device,
                 iters=400, fps=100.0,
                 w_marker=1.0, w_pose=1e-3, w_betas=5e-4,
                 w_transl_vel=1e-2, w_transl_acc=1e-3,
                 w_pose_vel=0.02, w_pose_acc=0.005,
                 w_spine_vel=0.05, w_spine_acc=0.01,
                 w_foot_orient=0.01, w_foot_smooth=0.03,
                 two_pass=True, smooth_sigma=1.5):
    ...
    def update_travel_from_current_transl():
        ...
        return (torch.from_numpy(td).float().to(device),
                torch.from_numpy(sp).float().to(device))
    ...
    fit_sequence_single_pass(
        fitter, markers_torch, targets_idx, travel_direction_torch, travel_speed_torch,
        device, iters, w_marker, w_pose, w_betas,
        w_transl_vel, w_transl_acc, w_pose_vel, w_pose_acc,
        w_spine_vel, w_spine_acc, w_foot_orient, w_foot_smooth,
        verbose=True
        ...
        if frames_to_check:
            ...
            with torch.no_grad():
                ...
                for frame_idx in frames_to_check:
                    ...
                    fitter.body_pose.data[frame_idx] = (
                        blend * ref_pose + (1 - blend) * fitter.body_pose.data[frame_idx]
                    )
            ...
            fit_sequence_single_pass(
                fitter, markers_torch, targets_idx, travel_direction_torch, travel_speed_torch,
                device, iters // 2,
                w_marker, w_pose, w_betas,
                w_transl_vel * 2, w_transl_acc * 2,
                w_pose_vel * 3, w_pose_acc * 3,
                w_spine_vel * 2, w_spine_acc * 2,
                w_foot_orient, w_foot_smooth * 2,
                verbose=True
            )
    ...
    final_marker_loss = fit_sequence_single_pass(
        fitter, markers_torch, targets_idx, travel_direction_torch, travel_speed_torch,
        device, iters // 4,
        w_marker * 2, w_pose, w_betas,
        w_transl_vel, w_transl_acc,
        w_pose_vel, w_pose_acc,
        w_spine_vel, w_spine_acc,
        w_foot_orient, w_foot_smooth,
        verbose=True
    )
...
def main():
    ...
    subjects = [args.subject] if args.subject else sorted(
        [p.name for p in processed.iterdir() if p.is_dir()]
    )
    ...
    for subj in subjects:
        ...
        if init_betas is None:
            ...
            result = fit_sequence(
                smpl, markers_torch, marker_ids, used_names,
                init_betas=None, device=device, fps=fps,
                iters=min(args.iters * 2, 1000),
                w_pose_vel=args.w_pose_vel, w_pose_acc=args.w_pose_acc,
                w_spine_vel=args.w_spine_vel, w_spine_acc=args.w_spine_acc,
                w_foot_orient=args.w_foot_orient, w_foot_smooth=args.w_foot_smooth,
                two_pass=not args.no_two_pass, smooth_sigma=args.smooth_sigma,
            )
        ...
        for npz_path in npz_files:
            ...
            result = fit_sequence(
                smpl, markers_torch, marker_ids, used_names,
                init_betas=init_betas, device=device, fps=fps, iters=args.iters,
                w_pose_vel=args.w_pose_vel, w_pose_acc=args.w_pose_acc,
                w_spine_vel=args.w_spine_vel, w_spine_acc=args.w_spine_acc,
                w_foot_orient=args.w_foot_orient, w_foot_smooth=args.w_foot_smooth,
                two_pass=not args.no_two_pass, smooth_sigma=args.smooth_sigma,

...
def generate_joints_from_smpl(smpl, poses, trans, betas):
    ...
    with torch.no_grad():
        output = smpl(
            global_orient=global_orient_t, body_pose=body_pose_t, betas=betas_t, transl=trans_t, pose2rot=True
        )
...
def main():
    ...
    smpl = smplx.create(model_path=args.model_dir, model_type="smpl", gender=gender, use_pca=False, num_betas=10)
    smpl.eval()
    ...
    ax.view_init(elev=15.0, azim=-90)
    ...
    # UI state
    ui_state = {"frame": 0, "paused": False, "anim": None, "box_size": box_size}
